...
echo_client(FILE *fp, int sockfd, const struct sockaddr *pservaddr, socklen_t servlen)
{
    ...
    signal(SIGALRM, sig_alrm);      // register signal handle sig_alrm for SIGALRM
    siginterrupt(SIGALRM, 1);       // SIG_ALRM signal interrupts system calls and the call returns -1 with errno EINTR
